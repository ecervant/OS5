...
void page_fault_handler( struct page_table *pt, int page )
{
  ...
  // if bits == 0 and frame table isn't full --> pick a unset frame, set it to the page
    if ( bits == 0 && full == 0){
        int i;
        for(i = 0; i < nframes; i++){
            if (frameTable[i] == -1){
...
//        printf("writes 1: %d\n", total_writes);
  ...
  // if bits == 0 but frame table is full --> algorithms
...
//            if (bits == (PROT_READ|PROT_WRITE)) {
  ...
  //          }
            ...
            exit(1);
                ...
                // fifo --> replacement_frames[frames] = frame 
/*
        disk_write(disk, kick_out, &physmem[new_frame*PAGE_SIZE]);
        total_writes++;
        disk_read(disk, page, &physmem[new_frame*PAGE_SIZE]);
